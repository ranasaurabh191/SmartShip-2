...
public class AuthServiceTests
{
    ...
    [Fact]
    public async Task FixAdmin_AdminDoesNotExist_ShouldCreateAdmin()
    {
        _userRepositoryMock
            .Setup(x => x.GetByEmailForAdminAsync("admin@smartship.com"))
            .ReturnsAsync((User?)null);
        ...
        var result = await _authService.FixAdminAsync();
    ...
    [Fact]
    public async Task FixAdmin_AdminExists_ShouldResetPassword()
    {
        var oldPasswordHash =
            BCrypt.Net.BCrypt.HashPassword("OldPassword@123");
        ...
        await _authService.FixAdminAsync();

        Assert.NotEqual(oldPasswordHash, admin.PasswordHash);
